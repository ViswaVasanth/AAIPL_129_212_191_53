...
class AnsweringAgent(object):
    ...
    def build_prompt(self, question_data: Dict[str, str|Any]) -> Tuple[str, str]:
        ...
        sys_prompt1 = "You are a world-class competitive exam solver with 99%+ accuracy. You excel at systematic logical reasoning, pattern recognition, and avoiding common traps. Approach each question methodically, analyze all options carefully, and never rush to conclusions. Your reputation depends on getting every answer correct."
        sys_prompt2 = (
            "You are an expert MCQ solver trained to handle adversarial questions designed to trick other AI systems. "
            "You recognize common logical fallacies, resist confidence bias, ignore misleading patterns, and focus purely on logical correctness. "
            "You systematically verify each answer against all constraints before making your final choice. Accuracy is your only priority."
        )
        
        tmpl = (
            # 'CRITICAL: Output ONLY valid JSON. Invalid JSON = DISQUALIFIED.\n\n'
            
            'Solve this MCQ accurately:\n\n'
            ...
            "FORMAT: END YOUR ANSWER as `FINAL ANSWER: [CORRECT OPTION]`"
            'Where [CORRECT_OPTION] is the letter (A, B, C, or D) of the actually correct choice.\n'
            "THERE SHOULD NOT BE ANY TEXT AFTER THE FINAL ANSWER"
        )
        
        prompt = tmpl.format(
            question=question_data['question'],
            choices=self._format_choices(question_data['choices']),
        )
        
        return prompt, sys_prompt1 if self.select_prompt1 else sys_prompt2

    def extract_json(self, answer):
        answer_json ={"answer": '', "reasoning": ''}
        if "FINAL ANSWER:" in answer:
            ...
            answer_json['answer'] = answer.split('FINAL ANSWER:')[1].upper()
            answer_json['reasoning'] = answer.split('FINAL ANSWER:')[0]
    ...
    def answer_question(self, question_data: Dict|List[Dict], **kwargs) -> Tuple[List[Dict], int|None, float|None]:
        ...
        if isinstance(resp, list):
            resp = [self.extract_json(r) for r in resp]
        else:
    ...
    for idx, (q, a) in enumerate(zip(sample_questions, answer)):
        ...
        try:
            ...
            if all(k in a for k in ['answer', 'reasoning']):
                # IMPROVED: Use robust answer letter extraction
                if len(a['answer']) != 1:
                    from utils.robust_json_processor import extract_answer_letter
                    a['answer'] = extract_answer_letter(a['answer'], q['choices'], agent.agent)
            else:
            ...
            # IMPROVED: Use robust JSON processor for malformed JSON
            from utils.robust_json_processor import extract_answer_json
            a = extract_answer_json(a, agent.agent)
        ans.append(a)
